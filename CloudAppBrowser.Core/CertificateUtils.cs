...
namespace CloudAppBrowser.Core
{
    public static class CertificateUtils
    {
        ...
        /// </summary>
        /// <remarks>Source: http://www.codeproject.com/Articles/162194/Certificates-to-DB-and-Back </remarks>
        ...
        public static RSACryptoServiceProvider DecodeRsaPrivateKey(byte[] privateKeyBytes)
        {
            ...
            try
            {
                ...
                parms.ProviderType = ((Environment.OSVersion.Version.Major > 5) || ((Environment.OSVersion.Version.Major == 5) && (Environment.OSVersion.Version.Minor >= 1)))
                    ? 0x18
                    : 1;
        ...
        public static X509Certificate2 ReadCertificateWithKey(string certFilename, string keyFilename, string password)
        {
            byte[] certBuffer = GetBytesFromPEM(File.ReadAllText(certFilename));
            byte[] keyBuffer = GetBytesFromPEM(File.ReadAllText(keyFilename));

            X509Certificate2 certificate = new X509Certificate2(certBuffer, password);

            RSACryptoServiceProvider prov = CertificateUtils.DecodeRsaPrivateKey(keyBuffer);
            certificate.PrivateKey = prov;

            return certificate;
        }

        private static byte[] GetBytesFromPEM(string text)
        {
